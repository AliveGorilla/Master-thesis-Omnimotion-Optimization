...
class RAFTExhaustiveDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(cached_flow_pred_files) > 0 and self.args.use_error_map:
            ...
            # option 1: sum of error map
            # option 2: random in error map (original) --> remove comments above and below
            select_ids = select_ids_error#np.random.choice(np.concatenate([select_ids_error, select_ids_random]), self.num_pts,
                                          #replace=False)
        else:
