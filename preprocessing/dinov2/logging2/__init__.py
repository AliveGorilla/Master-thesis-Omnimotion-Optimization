...
def _configure_logger(
    ...
    *,
    ...
    datefmt = "%Y%m%d %H:%M:%S"
    formatter = log2.Formatter(fmt=fmt, datefmt=datefmt)

    # stdout logging for main worker only
    if distributed.is_main_process():
        handler = log2.StreamHandler(stream=sys.stdout)
        handler.setLevel(log2.DEBUG)
    ...
    if output:
        ...
        handler = log2.StreamHandler(open(filename, "a"))
        handler.setLevel(log2.DEBUG)
        handler.setFormatter(formatter)
        logger.addHandler(handler)
...
def setup_logging(
    ...
    *,
    ...
    """
    log2.captureWarnings(capture_warnings)
