...
def train(args):
    ...
    print("Started Time Formatted UTC Time:",strf)
    ...
    print("Started Time Formatted UTC Time:",strf)
    end = time.gmtime(time.time())
    endf = time.strftime("%D %T", end)
    print("Finish Time Formatted UTC Time:",endf)
    print("Total Time Taken:",(time.time()-start)/3600,"Hours")

    # save running time into txt file
    f = os.path.join(out_dir, 'running_time.txt')
    with open(f, 'w') as file:
        file.write("Started Time Formatted UTC Time: "+strf+"\n")
        file.write("Finish Time Formatted UTC Time: "+endf+"\n")
        file.write("Total Time Taken: "+str((time.time()-start)/3600)+" Hours\n")
