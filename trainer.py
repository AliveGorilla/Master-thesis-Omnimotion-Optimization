...
from networks.mfn_pe import GaborNetPE
...
class BaseTrainer():
    def __init__(self, args, device='cuda'):
        # init the tcnn config
        with open('./config_hash.json') as f:
            self.tcnn_config = json.load(f)
        ...
        self.feature_mlp = tcnn.NetworkWithInputEncoding(n_input_dims=self.n_encodings, n_output_dims=self.n_channels, 
                                                     encoding_config=self.tcnn_config["encoding"], 
                                                     network_config=self.tcnn_config["network"]).to(device)
        ...
        self.color_mlp = GaborNetPE(in_size=3,
                                  hidden_size=256,
                                  n_layers=3,
                                  alpha=3,
        ...
        self.feature_list = torch.empty(128).to(device)
    ...
    def get_prediction_one_way(self, x, id, inverse=False):
        ...
        freezing technique is used to avoid overfitting after treshold steps
        :param x: [n_imgs, n_pts, n_samples, 3]
        :param id: [n_imgs, ]
        :param inverse: True or False
        :return: [n_imgs, n_pts, n_samples, 3]
        '''
        treshold = self.args.freeze_step
        # option 1: t, log(t+1)
        t = self.time_steps[id] # [n_imgs, 1]  # [8, 1]
        log_t = torch.log(t + 1)
        '''
        option 2: t, sin(t), cos(t)
        sin_t = torch.sin(t * 2 * np.pi)
        cos_t = torch.cos(t * 2 * np.pi)
        '''
        if self.step >= treshold:
            feature = self.feature_list[id]  # [n_imgs, n_feat]
        else:
            # option 1: t, log(t+1)
            feature = self.feature_mlp(torch.cat((t, log_t), dim=1))
            # option 2: t, sin(t), cos(t)
            #feature = self.feature_mlp(torch.cat((t, sin_t, cos_t), dim=1))
    ...
    def train_one_step(self, step, batch):
        ...
        if self.step == treshold:
            ...
            for param in self.feature_mlp.parameters():
                param.requires_grad = False
            # option 1: t, log(t+1)
            self.feature_list = torch.concatenate([self.feature_mlp(torch.cat((t, torch.log(t+1)), dim=0).unsqueeze(0)) 
                                                   for t in self.time_steps])
            # option 2: t, sin(t), cos(t)
            '''
            self.feature_list = torch.concatenate([self.feature_mlp(torch.cat((t, torch.sin(t * 2 * np.pi), 
                                                                              torch.cos(t * 2 * np.pi)), dim=0).unsqueeze(0))
                                                                               for t in self.time_steps])
            '''
            print("Last gradient value: ", self.feature_list[-1])
        ...
        flag = time.time()
        loss.backward()
        global backwords_time
        backwords_time.append(round(time.time() - flag, 6))

        is_break = False
        for p in self.deform_mlp.parameters():
            if torch.isnan(p.data).any() or torch.isnan(p.grad).any():
                is_break = True
                print("Deform MLP has NaN values")

        if self.step < 20000:
            for p in self.feature_mlp.parameters():
                if torch.isnan(p.data).any() or torch.isnan(p.grad).any():
                    is_break = True
                    print("Feature MLP has NaN values")

        for p in self.color_mlp.parameters():
            if torch.isnan(p.data).any() or torch.isnan(p.grad).any():
                is_break = True
                print("Color MLP has NaN values")
    ...
    def get_globals_(self):
        mega_list_ = [backwords_time]
